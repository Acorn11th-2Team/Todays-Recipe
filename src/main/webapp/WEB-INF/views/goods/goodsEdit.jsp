	...
	<table style='padding: 5px; border: 1px; border-collapse: collapse'>
		...
		<tr>
			...
			<td align="center"><input type="hidden" name=gCategory
				...
				<c:forEach var="dto" items="${goodsEdit}" varStatus="status">
					<script>
						...
						if (result == 1 && count < num) {
							result = count;
						} else if (result == 1 && ${status.last}) {
							result = count+1;
						}
					</script>
				</c:forEach>
			...
			<td align="center" colspan=2><input type="button" value="상품 추가"
				style="width: 89px; height: 24px" class="addBtn"
				data-CodePrefix=<%=cm%> data-ImgPrefix=<%=ci%>></td>
		</tr>
		<tr height="30px" style="border-bottom: 2px double #444444;">
			<td align="center" colspan=8><b style="font-size: 20px">상품
					목록</b></td>
		</tr>
		<c:forEach var="dto" items="${goodsEdit}" varStatus="status">
			<tr>
				...
				<td class="td_default"><input type="text" name=gPrice
					id="gPrice${dto.gCode}" value="${dto.gPrice}" size=8 maxlength=8></td>
				<td class="td_default"><input type="text" name=gStock
					id="gStock${dto.gCode}" value="${dto.gStock}" size=6 maxlength=5></td>
				<td><input type="button" value="수정" class="updateBtn"
					data-gCode="${dto.gCode}"></td>
