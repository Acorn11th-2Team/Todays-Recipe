...
<div class="page-wrapper">
    <div class="container-fluid">
        <div class="col-lg-20"><!--게시판 넓이 -->
            ...
            <div class="row">
                  <div class="col-lg-12">
                      <c:if test="${ not empty login.userid && login.userid eq 'admin' }">
                      <button id="noticewriting" type="button" class="btn btn-info pull-right">
                    ...
                    <table class="table table-hover">
                        ...
                        <c:forEach var="noticelist" items="${viewAll}">
                            <tr>
                                ...
                                <td><fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${noticelist.writeday}"/></td>
...
<div style="display: block; text-align: center;">		
		<c:if test="${paging.startPage != 1 }">
			<a href="notice?nowPage=${paging.startPage - 1 }&countPerPage=${paging.countPerPage}">&lt;</a>
		</c:if>
		<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
			<c:choose>
				...
				<c:when test="${p != paging.nowPage }">
					<a href="notice?nowPage=${p }&countPerPage=${paging.countPerPage}">${p }</a>
				</c:when>
			</c:choose>
		</c:forEach>
		<c:if test="${paging.endPage != paging.lastPage}">
			<a href="notice?nowPage=${paging.endPage+1 }&countPerPage=${paging.countPerPage}">&gt;</a>
		</c:if>
	</div>
